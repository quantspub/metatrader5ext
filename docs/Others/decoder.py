...
from nautilus_metatrader5.common import MarketDataTypeEnum
from nautilus_metatrader5.symbol import SymbolInfo
        ...
        match fn_name.lower():
            ...
            case "managed_accounts":
                task = self.create_task(
                    self.handler.process_managed_accounts, accounts=response
                )
            case "req_ids":
                order_id = response[0]
                task = self.create_task(
                    self.handler.process_next_valid_id, order_id=order_id
                )
            ...
            case "req_symbol_details" | "req_matching_symbols":  # |
                task = self.create_task(
                    self.handler.process_symbol_details,
                    req_id=req_id,
                    symbol_infos=response,
                )
