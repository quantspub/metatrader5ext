...
string MqlBookToString(MqlBookInfo &Book[]){
   string result = "";
   datetime data = TimeCurrent();
   int size_book = ArraySize(Book);
   for(int i = 0; i < size_book; i++){
      if(i != 0) result += ";";
      result += IntegerToString(Book[i].type) + ",";
      result += DoubleToString(Book[i].price, _Digits_) + ",";
      result += IntegerToString(Book[i].volume) + ",";
      result += DoubleToString(Book[i].volume_real, _Digits_) + ",";
      result += TimeToString(data,TIME_DATE|TIME_MINUTES|TIME_SECONDS);
   }
   return result;
}
...
// CopyBook
////////////////////////////////////////////////////////////////////////////////////////////
string CopyBookToString(string symbol){
   MarketBookAdd(symbol);
   MqlBookInfo book[];
   MarketBookGet(symbol, book);
   MarketBookRelease(symbol);
   return MqlBookToString(book);
}
